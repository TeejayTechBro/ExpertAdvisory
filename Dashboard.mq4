...
int OnInit()
  {
   ...
   EventSetMillisecondTimer(60000); // M1 candle update
   ...
   for (int idx = 0; idx < SYMBOL_COUNT; idx++)
     {
      ...
      if (ArraySize(dailyMovers) <= idx) ArrayResize(dailyMovers, idx + 1);
      dailyMovers[idx] = mover;

      double change = ((current - openPrice) / openPrice) * 100;
      if (ArraySize(dailyChanges) <= idx) ArrayResize(dailyChanges, idx + 1);
      ...
      for (int idx2 = idx1 + 1; idx2 < SYMBOL_COUNT; idx2++)
        {
         ...
         if (ArraySize(correlations) <= idx1 * SYMBOL_COUNT + idx2) ArrayResize(correlations, idx1 * SYMBOL_COUNT + idx2 + 1);
...
double CalculateCorrelation(string symbol1, int timeframe, string symbol2, int period, int shift)
  {
   ...
   if (copied1 != period || copied2 != period) return 0; // Not enough data
   ...
   if (denominator1 == 0 || denominator2 == 0) return 0; // Prevent division by zero
